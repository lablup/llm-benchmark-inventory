...
- [서빙 보드별 대표 측정값 CSV](./serving_reference_values.csv)
- [서빙 보드별 대표 측정값 조사 기록](./docs/serving-reference-values.md)

## 분야 구분

- **한국어·영어 벤치마크**는 모델 능력(model capability)을 평가하는 데이터셋과 실행 도구를 다룹니다.
- **서빙·자원**은 지연 시간과 처리량을 재는 도구, 공개 성능 비교, GPU·엔진 계측을 다룹니다.
- 점수는 출처와 확인일을 함께 적습니다. 벤치마크 카드의 **추천 모델**은 한 출처·한 설정 안에서 점수가 높은 모델 3개와 공개 가중치(open-weight) 모델 3개를 점수, 모델 링크, 출처 링크, 기준일과 함께 보여줍니다. 서빙·자원 자료는 공식 방법론과 결과 페이지를 연결하고 재현 조건과 운영 상태를 기록하며, 공개 성능 표준·보드 카드에는 모델·하드웨어·엔진·부하 조건을 함께 적은 **대표 측정값** 3개를 실었습니다. 조사 방법과 한계는 [벤치마크별 추천 모델 조사 기록](./docs/recommended-models.md)에 있습니다.

## 읽는 방법

- **상업적 이용**은 데이터 카드 등에 표시된 라이선스를 기준으로 `가능`, `불가`, `확인 불가`, `조건 확인 필요`의 네 가지로 구분합니다.
- 라이선스는 2026-09-26에 각 데이터 카드와 저장소에서 확인했습니다. 이후 바뀔 수 있으므로 실제 이용 전에 원문을 다시 확인해야 합니다.
...
총 57개 · 상업적 이용 가능 28개 · 불가 12개 · 확인 불가 11개 · 조건 확인 필요 6개
...
| [K-MMBench](https://huggingface.co/datasets/NCSOFT/K-MMBench) | 이미지(종합 이해) | 영어 MMBench를 한국어로 번역하고 확장한 종합 이미지 이해 평가. 4,329개 객관식 문항으로 사물 위치, 속성, 관계와 추론 등 20개 범주를 다룸 | **dev**<br>도표나 사진을 보고 물체의 속성, 위치와 관계를 파악한 뒤 두 개 또는 네 개 선택지 가운데 정답을 고름 | **불가** | CC-BY-NC-4.0<br>수정·파생: 가능(조건 준수)<br>재배포: 조건부 가능<br>한국에서 수집한 원본 평가가 아니라 번역·확장판이므로 한국 문화 이해와 별도로 해석해야 함 | 없음 |
...
| [KMMMU](https://huggingface.co/datasets/HAERAE-HUB/KMMMU) | 이미지(학문·시험 추론) | 한국 공무원 시험, 국가기술자격시험, 국가직무능력표준(NCS) 시험과 학술 올림피아드에서 수집한 한국어 멀티모달 평가. 문항 3,466개와 이미지 3,628개로 구성되며 이미지 안에 글자가 있는 문항이 2,550개 | **Korean-specific**<br>한국 시험 문항의 도표, 사진과 본문을 함께 읽고 정답을 고르는 객관식 평가. 여러 이미지가 붙은 문항도 포함 | **불가** | CC-BY-NC-4.0<br>수정·파생: 가능(조건 준수)<br>재배포: 조건부 가능<br>이미지 링크를 실행 시점에 내려받아야 하므로 에어갭 실행 전 자산 고정 필요 | 없음(공식 평가 코드 별도) |
| [KRETA](https://huggingface.co/datasets/tabtoyou/KRETA) | 이미지(문서·문자 추론) | 한국의 실제 환경에서 촬영한 글자가 많은 이미지로 문자 읽기와 추론을 평가. 2,577개 문항을 15개 분야와 26개 이미지 유형으로 나누고 단순 읽기(System 1)와 여러 단계 추론(System 2)을 구분 | **System 1·System 2**<br>행사 현수막에서 문구를 읽거나 행사 장소와 가장 가까운 역을 이미지 정보와 외부 지식을 함께 사용해 고르는 객관식 평가 | **확인 불가** | 미표기<br>수정·파생: 확인 불가<br>재배포: 확인 불가<br>Hugging Face 데이터 카드와 공식 저장소에서 데이터 라이선스를 찾지 못함. 이용 전 저자 확인 필요 | 없음(공식 평가 코드 별도) |
...
| [KSAFE-MM](https://huggingface.co/datasets/K-intelligence/KSAFE-MM) | 이미지 안전(유해·거부) | 한국어와 한국 문화 맥락에서 멀티모달 모델의 안전한 응답을 평가. 일반 위험을 다루는 KSAFE-MM-G 1,650건과 한국 문화에 맞춘 합성 이미지 기반 KSAFE-MM-C 12,485건으로 구성 | **KSAFE-MM-G·KSAFE-MM-C**<br>이미지만 있는 경우, 이미지 안에 문구가 있는 경우와 이미지·문구가 함께 있는 경우에 유해 요청을 따르는지, 안전한 요청까지 지나치게 거부하는지를 측정 | **불가** | CC-BY-NC-4.0<br>수정·파생: 가능(조건 준수)<br>재배포: 조건부 가능<br>유해하고 민감한 이미지와 문장이 포함되어 있으므로 접근 통제와 결과 로그 비식별화 필요 | 없음 |
...
| 벤치마크 | 평가 축 | 설명 | 대표 예시 | 상업적 이용 | 라이선스·이용 조건 | lm-eval |
|---|---|---|---|---|---|---|
| [HAERAE-VISION](https://huggingface.co/datasets/HAERAE-HUB/HAERAE-VISION) | 이미지(실사용 질의) | 한국 온라인 커뮤니티의 실제 질문에서 만든 시각 질의응답 평가. 같은 이미지에 설명이 빠진 원래 질문(original)과 맥락을 명확히 쓴 질문(explicit)을 함께 제공해 질문의 불명확성이 성능에 미치는 영향을 비교 | **original·explicit**<br>게임 화면을 보고 '어린 용 세 마리 말고 더 있나요?'처럼 맥락을 생략한 질문과 대상을 분명히 밝힌 질문에 각각 답함 | **가능** | CC-BY-4.0<br>수정·파생: 가능(조건 준수)<br>재배포: 조건부 가능<br>GPT-5-mini를 기본 판정 모델로 쓰는 공식 평가 코드가 별도로 있으며 API 키 필요 | 없음(공식 평가 코드 별도) |
| [KOFFVQA](https://huggingface.co/datasets/maum-ai/KOFFVQA_Data) | 이미지(자유형 VQA) | 일반 장면에 관한 한국어 자유형 답변을 10개 과제와 세 가지 상위 범주로 평가하는 벤치마크. 275개 이미지·질문 쌍마다 답변을 세부 기준으로 나누어 점수를 주는 채점표를 제공 | **perception**<br>사진에 비행기가 있는지와 몇 대인지 묻고, 비행기 존재 판단과 정확한 개수에 각각 점수를 부여 | **가능** | Apache-2.0<br>수정·파생: 가능(조건 준수)<br>재배포: 조건부 가능<br>자유형 응답을 채점하려면 공식 코드에서 LLM 판정 모델을 실행해야 함 | 없음(공식 평가 코드 별도) |
| [K-DTCBench](https://huggingface.co/datasets/NCSOFT/K-DTCBench) | 이미지(한국어 문서) | 한국어로 작성된 문서, 표와 차트를 읽는 능력을 평가하기 위해 직접 만든 240개 객관식 문항. 문서(document), 표(table), 차트(chart)의 세 범주로 구성 | **document**<br>보고서의 주요 내용이 아닌 항목을 네 선택지에서 고르거나 계약서 이미지에서 보증금과 월 임대료를 찾음 | **불가** | CC-BY-NC-4.0<br>수정·파생: 가능(조건 준수)<br>재배포: 조건부 가능<br>비상업 조건이 있어 제품 평가에 사용하기 전에 이용 범위 확인 필요 | 없음 |
